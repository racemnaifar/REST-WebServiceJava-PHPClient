...
public function add(){
	$data = array("lib" => "$this->lib", "categorie" => "$this->categorie", "couleur"=>"$this->couleur", "qte"=>"$this->qte");                                                                    
...
public function detail(){
	$url = "http://localhost:8080/FleuristeApp/webresources/fleur/GetByCode?code=".$this->code;
	$ch = curl_init($url);
	curl_setopt($ch, CURLOPT_RETURNTRANSFER,true);
	curl_setopt($ch, CURLOPT_CUSTOMREQUEST,"GET");
	$response = curl_exec($ch);
